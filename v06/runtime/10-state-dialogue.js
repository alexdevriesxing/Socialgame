function resetGame(){
  ...
  game.scheduleStatus={};game.dayFlags={talked:[],hotspots:[],help:0,club:0,ontime:0,event:false,brief:false,challenge:false,message:false,showcase:false};game.dailyBrief=null;
...
function gameSnapshot(){
  return {player:game.player,year:game.year,month:game.month,day:game.day,time:game.time,
    ...
    finished:game.finished,dailyBrief:game.dailyBrief,promResult:game.promResult||null,promCompanion:game.promCompanion||null};
}
function saveGame(){
...
function loadGame(){
  try{
    ...
    game.player.showcaseWins ??= 0; game.player.eventPrestige ??= 0;
    game.dayFlags ||= {talked:[],hotspots:[],help:0,club:0,ontime:0,event:false,brief:false,challenge:false};
    game.dayFlags.brief ??= false; game.dayFlags.challenge ??= false; game.dayFlags.message ??= false; game.dayFlags.showcase ??= false; game.dailyBrief ||= createDailyBrief(game.year,game.month,game.day,game.player.club);
...
function openDialogue({speaker='Narrator',portrait=null,scene=null,text,choices=[],onClose=null}){
  game.dialogue={speaker,portrait,scene,text,choices,onClose,reveal:0,selected:0,opened:performance.now()};
  audio.click();
}
function closeDialogue(choiceIndex=0){
  ...
  if(choice){
    ...
    if(result) setTimeout(()=>openDialogue({speaker:choice.resultSpeaker||d.speaker,portrait:d.portrait,scene:d.scene,text:result,choices:[{text:'Continue',action:()=>{if(action)action();if(after)after();}}]}),30);
