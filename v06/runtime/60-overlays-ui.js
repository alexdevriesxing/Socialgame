...
function drawHelpOverlay(o){
  ...
  lines.forEach((l,i)=>wrapped(l,95,87+i*27,770,12,i%2?COLORS.cream:COLORS.paper,2));
  addButton(365,465,230,32,'CLOSE',()=>game.overlay=null);
}
...
function drawMonthEnd(o){
  ...
  centered(showcase?`Showcase Grade ${showcase.grade} • ${showcase.score}% • Event prestige ${game.player.eventPrestige}`:'Showcase record unavailable',440,13,COLORS.gold,`bold 13px ${UI_FONT}`);
...
function drawProm(){
  ...
  centered(`Showcase wins: ${game.player.showcaseWins} • Event prestige: ${game.player.eventPrestige}`,294,14,COLORS.gold,'bold 14px Trebuchet MS');
  wrapped(`The crown is only one ending. Every friendship, message, showcase, apology, brave choice and ordinary day became part of your school legend. ${game.promCompanion||'Your closest friends'} stands beside you as the final song begins.`,190,330,580,16,COLORS.cream,5);
