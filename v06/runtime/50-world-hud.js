...
function drawWorldCharacters(){
  ...
  for(const a of actors){
    ...
    const row=a.type==='player'?game.player.direction:0,col=a.type==='player'?game.player.anim:(Math.floor((performance.now()+a.x*7)/1100)%2?3:4);
...
function drawDialogue(){
  ...
  if(d.scene!==null&&d.scene!==undefined&&images.event_atlas){
    const ex=(d.scene%4)*200,ey=Math.floor(d.scene/4)*200;
