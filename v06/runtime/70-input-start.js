...
window.addEventListener('keydown',e=>{
  ...
  if(['arrowup','arrowdown','arrowleft','arrowright',' ','e','q','r','j','p','k'].includes(k))e.preventDefault();
  if(game.dialogue){
    if(k===' '||k==='e'||k==='enter'){
  ...
  if(game.overlay){if(k==='escape'||k==='q'||k==='r'||k==='j'||k==='p'||k==='k')game.overlay=null;return;}
  if(game.mode==='play'){
    if(k==='e'||k===' ')interact();
