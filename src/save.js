...
export const SAVE_KEY = 'sakura-crest-social-summit';
export const LEGACY_SAVE_KEYS = ['sakura-crest-social-summit-v1'];
...
export function migrateSave(raw) {
  ...
  // The original vertical slice stored the snapshot directly without an envelope.
...
export function readSave(storage) {
  ...
  for (const key of candidates) {
    ...
    if (key !== SAVE_KEY || migrated.version !== CURRENT_SAVE_VERSION) {
      writeSave(storage, migrated.data);
    }
