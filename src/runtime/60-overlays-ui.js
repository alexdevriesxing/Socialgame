function overlayBase(title){panel(50,30,860,480,COLORS.ink,COLORS.gold,4);centered(title.toUpperCase(),64,26,COLORS.gold,'bold 26px Courier New');}
...
function drawClubOverlay(o){
  ...
  centered(`${currentClubRank()} • ${game.player.clubXP} XP • Prestige ${game.player.clubPrestige}`,102,16,club.color,'bold 16px Courier New');
  ...
  wrapped(`Next major event: ${club.competitions[Math.min(3,game.year-1)]}`,516,350,330,13,COLORS.paper,3);
  addButton(365,448,230,34,'CLOSE',()=>game.overlay=null);
}
...
function drawHelpOverlay(o){
  ...
  lines.forEach((l,i)=>wrapped(l,95,118+i*40,770,15,i%2?COLORS.cream:COLORS.paper,2));
  addButton(365,465,230,32,'CLOSE',()=>game.overlay=null);
}
function drawDayEnd(o){
  overlayBase(o.title);centered('THE FINAL BELL',103,18,COLORS.sky,'bold 18px Courier New');
...
function drawMonthEnd(o){
  ...
  centered(`${MONTHLY_SPECIALS[(game.month-1)%12].event}`,99,16,COLORS.sky,'bold 16px Courier New');
  ...
  centered(`You finished #${p.position} with ${p.score} points.`,398,19,COLORS.gold,'bold 19px Courier New');
  centered(`Year ${game.year}, month ${game.month} of 48 complete.`,425,14,COLORS.cream,'bold 14px Courier New');
  addButton(330,455,300,40,game.year===4&&game.month===12?'GO TO PROM':'CONTINUE TO NEXT MONTH',continueMonth);
}
...
function drawProm(){
  ...
  panel(120,90,720,350,COLORS.panel,COLORS.gold,5);
  centered('SAKURA CREST PROM',132,34,COLORS.gold,'bold 34px Courier New');
  centered(game.promResult||'Four years become one final night.',207,21,COLORS.paper,'bold 21px Courier New');
  centered(`Monthly results recorded: ${game.player.monthlyScores.length}`,270,15,COLORS.sky,'bold 15px Courier New');
  wrapped('The crown is only one ending. Every friendship, apology, brave choice and ordinary day became part of your school legend.',190,315,580,17,COLORS.cream,5);
  addButton(330,390,300,38,'RETURN TO TITLE',()=>{game.mode='title';game.overlay=null;});
}

function drawNotifications(){
  game.notifications.forEach((n,i)=>{
    const alpha=clamp(n.life,0,1);ctx.globalAlpha=alpha;const y=18+i*28;
    ctx.font='bold 13px Courier New';const w=Math.min(600,ctx.measureText(n.text).width+26);
...
function addButton(x,y,w,h,labelText,onClick,enabled=true,selected=false,color=COLORS.blue,opts={}){
  ...
  centeredAt(labelText,x+w/2,y+h/2+5,Math.min(15,h*.42),selected?COLORS.ink:COLORS.paper,`bold ${Math.min(15,h*.42)}px Courier New`);
...
function label(t,x,y,w,h,color){ctx.fillStyle=COLORS.shadow;ctx.fillRect(x,y,w,h);ctx.strokeStyle=color;ctx.strokeRect(x+.5,y+.5,w-1,h-1);centeredAt(t,x+w/2,y+h/2+4,10,color,'bold 10px Courier New');}
function text(t,x,y,size=14,color=COLORS.paper,bold=false){ctx.font=`${bold?'bold ':''}${size}px Courier New`;ctx.fillStyle=color;ctx.textBaseline='alphabetic';ctx.fillText(String(t),x,y);}
function centered(t,y,size=14,color=COLORS.paper,font=null){ctx.font=font||`${size}px Courier New`;ctx.fillStyle=color;ctx.textAlign='center';ctx.fillText(String(t),W/2,y);ctx.textAlign='left';}
function centeredAt(t,x,y,size=14,color=COLORS.paper,font=null){ctx.font=font||`${size}px Courier New`;ctx.fillStyle=color;ctx.textAlign='center';ctx.fillText(String(t),x,y);ctx.textAlign='left';}
function wrapped(t,x,y,maxW,size=14,color=COLORS.paper,maxLines=6){
  ctx.font=`${size}px Courier New`;ctx.fillStyle=color;const paras=String(t).split('\n');let lineY=y,used=0;
