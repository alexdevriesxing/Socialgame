...
function drawTitle(){
  ...
  centered('SAKURA CREST',65,40,COLORS.paper,'bold 42px Courier New');
  centered('SOCIAL SUMMIT',109,24,COLORS.pink,'bold 25px Courier New');
  centered('Four school years. Forty-eight rankings. One unforgettable prom.',164,18,COLORS.paper,'bold 18px Courier New');
  ...
  centered('WASD / Arrows • E / Space interact • R rankings • Q schedule',486,14,COLORS.cream,'bold 14px Courier New');
  centered('Original family-friendly pixel art • Vertical Slice v0.1',515,12,COLORS.sky,'12px Courier New');
}
...
function drawCreator(){
  ...
  centered('CREATE YOUR STUDENT',60,28,COLORS.paper,'bold 28px Courier New');
...
function charCard(x,y,id,label,characterName,selected){
  ...
  drawCharacter(characterName,0,0,x+48,y+28,48,64);
  centeredAt(label,x+72,y+112,14,selected?COLORS.gold:COLORS.paper,'bold 14px Courier New');
...
function drawPlay(){
  ...
  ctx.drawImage(images.school_map,game.camera.x,game.camera.y,VIEW_W,H,0,0,VIEW_W,H);
