...
for(const target of targets){
  ...
  try{
    ...
    await page.waitForFunction(()=>document.getElementById('loading')?.classList.contains('hidden')&&window.SAKURA_RELEASE?.version==='1.10.0',null,{timeout:20000});
    await waitForPaint(page,`${target.label} title`);
    const boot=await page.evaluate(()=>({release:window.SAKURA_RELEASE?.version,saveVersion:CURRENT_SAVE_VERSION,readiness:validateReleaseReadiness(),social:window.SakuraSocialMemoryV19?.validate?.(),anime:validateAnimeArtV18(),realAssets:validateRealAssetCompletionV110(),canvas:{width:canvas.width,height:canvas.height},scripts:[...document.scripts].map(script=>script.getAttribute('src')).filter(Boolean).length}));
    if(boot.release!=='1.10.0'||boot.saveVersion!==10||!boot.readiness.valid||!boot.social?.valid||!boot.anime.valid||!boot.realAssets.valid||boot.anime.externalRuntimeDependency!==false||boot.anime.proceduralFallbacks!==false||boot.canvas.width!==960||boot.canvas.height!==540||boot.scripts<34)throw new Error(`${target.label} boot validation failed: ${JSON.stringify(boot)}`);
    ...
    const loaded=await page.evaluate(()=>({loaded:loadGame(),name:game.player.name,score:game.player.score,club:game.player.club,social:window.SakuraSocialMemoryV19?.validate?.(),recovery:getSaveRecoveryReport()}));
    if(!loaded.loaded||loaded.name!=='Engine QA'||loaded.score!==321||loaded.club!=='tech'||!loaded.social?.valid||loaded.recovery.failures.length)throw new Error(`${target.label} save/load failed: ${JSON.stringify(loaded)}`);
...
console.log(`Multi-engine v1.10 browser QA passed with Deep Social Memory and ten permanent artwork files: ${results.map(result=>result.label).join(', ')}.`);
