...
for(const target of targets){
  ...
  try{
    ...
    await page.waitForFunction(()=>document.getElementById('loading')?.classList.contains('hidden')&&window.SAKURA_RELEASE?.version==='1.11.0',null,{timeout:20000});
    await waitForPaint(page,`${target.label} title`);
    const boot=await page.evaluate(()=>({release:window.SAKURA_RELEASE?.version,saveVersion:CURRENT_SAVE_VERSION,readiness:validateReleaseReadiness(),social:window.SakuraSocialMemoryV19?.validate?.(),anime:validateAnimeArtV18(),realAssets:validateRealAssetCompletionV110(),living:validateLivingDistrictV111(),canvas:{width:canvas.width,height:canvas.height},scripts:[...document.scripts].map(script=>script.getAttribute('src')).filter(Boolean).length}));
    if(boot.release!=='1.11.0'||boot.saveVersion!==10||!boot.readiness.valid||!boot.social?.valid||!boot.anime.valid||!boot.realAssets.valid||!boot.living.valid||boot.living.signatureActivities!==14||boot.living.uniqueAmbientProfiles!==14||boot.anime.externalRuntimeDependency!==false||boot.anime.proceduralFallbacks!==false||boot.canvas.width!==960||boot.canvas.height!==540||boot.scripts<35)throw new Error(`${target.label} boot validation failed: ${JSON.stringify(boot)}`);
    const artResponses=await Promise.all(['keyart.webp','campus.webp','characters.webp','portraits.webp','objects.webp','district-map.webp','world-locations.webp','events.webp','rivals.webp','memories.webp'].map(name=>page.request.get(`${baseURL}/assets/anime/${name}`)));
    if(artResponses.some(response=>!response.ok()))throw new Error(`${target.label} could not load all permanent anime assets.`);
    await page.evaluate(()=>{resetGame();game.mode='play';game.player.name='Engine QA';game.player.gender='girl';game.player.club='tech';game.player.socialStatus='ordinary';initializeSocialStatus();ensureLivingDistrictProgress();writeSave(localStorage,gameSnapshot());openAccessibility(0);});
    ...
    const loaded=await page.evaluate(()=>({loaded:loadGame(),name:game.player.name,score:game.player.score,club:game.player.club,social:window.SakuraSocialMemoryV19?.validate?.(),living:validateLivingDistrictV111(),recovery:getSaveRecoveryReport()}));
    if(!loaded.loaded||loaded.name!=='Engine QA'||loaded.score!==321||loaded.club!=='tech'||!loaded.social?.valid||!loaded.living?.valid||loaded.recovery.failures.length)throw new Error(`${target.label} save/load failed: ${JSON.stringify(loaded)}`);
...
console.log(`Multi-engine v1.11 browser QA passed with Deep Social Memory, Living District progression and ten permanent artwork files: ${results.map(result=>result.label).join(', ')}.`);
